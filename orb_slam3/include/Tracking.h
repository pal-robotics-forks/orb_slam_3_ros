...
{
public:
  Tracking(System *pSys, ORBVocabulary *pVoc, Atlas *pAtlas, KeyFrameDatabase *pKFDB,
  ...
  // Use this function if you have deactivated local mapping and you only want to localize
  // the camera.
...
protected:
  ...
  // In case of performing only localization, this flag is true when there are no matches
  // to
  // points in the map. Still tracking will continue if there are enough matches with
  // temporal points.
  // In that case we are doing visual odometry. The system will try to do relocalization
  // to recover
