...
public:
  Point(const float &acc_x, const float &acc_y, const float &acc_z, const float &ang_vel_x,
        const float &ang_vel_y, const float &ang_vel_z, const double &timestamp)
...
public:
  ...
  void Set(const cv::Mat &Tbc_, const float &ng, const float &na, const float &ngw,
           const float &naw);
...
public:
  ...
  void IntegrateNewMeasurement(const cv::Point3f &acceleration, const cv::Point3f &angVel,
                               const float &dt);
  void Reintegrate();
