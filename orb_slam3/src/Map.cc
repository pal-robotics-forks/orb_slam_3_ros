...
void Map::ApplyScaledRotation(const cv::Mat& R, const float s, const bool bScaledVel,
                              const cv::Mat t)
{
  unique_lock<mutex> lock(mMutexMap);
...
void Map::printReprojectionError(list<KeyFrame*>& lpLocalWindowKFs, KeyFrame* mpCurrentKF,
...
void Map::PreSave(std::set<GeometricCamera*>& spCams)
{
  ...
  for (MapPoint* pMPi : mspMapPoints)
  {
         ...
         it != end; ++it)
    {
      ...
      {
        pMPi->EraseObservation(it->first);  // We need to find where the KF is set as Bad
                                            // but the observation is not removed
      }
    }
  }
