...
namespace ORB_SLAM3
{
LoopClosing::LoopClosing(Atlas* pAtlas, KeyFrameDatabase* pDB, ORBVocabulary* pVoc,
        ...
        if (mbMergeDetected)
        {
          ...
          else
          {
            ...
            if (mpCurrentKF->GetMap()->IsInertial() && mpMergeMatchedKF->GetMap()->IsInertial())
            {
              ...
              if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
                   mpTracker->mSensor == System::IMU_STEREO) &&
                  mpCurrentKF->GetMap()->GetIniertialBA1())  // TODO, maybe with
                                                             // GetIniertialBA1
              {
            ...
            //                        cout << "tw2w1: " << mSold_new.translation() <<
            //                        endl;
            //                        cout << "Rw2w1: " <<
            //                        mSold_new.rotation().toRotationMatrix() << endl;
            //                        cout << "Angle Rw2w1: " <<
          ...
          if (mpCurrentKF->GetMap()->IsInertial())
          {
            ...
            if (fabs(phi(0)) < 0.008f && fabs(phi(1)) < 0.008f && fabs(phi(2)) < 0.349f)
            {
              if (mpCurrentKF->GetMap()->IsInertial())
              {
                ...
                if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
                     mpTracker->mSensor == System::IMU_STEREO) &&
                    mpCurrentKF->GetMap()->GetIniertialBA2())  // TODO, maybe with
                                                               // GetIniertialBA1
                {
                  phi(0) = 0;
                  phi(1) = 0;
...
bool LoopClosing::NewDetectCommonRegions()
{
  ...
  // cout << "LC: bMergeDetectedInKF: " << bMergeDetectedInKF << "   bLoopDetectedInKF: "
  // << bLoopDetectedInKF << endl;
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF,
                                                 g2o::Sim3& gScw, int& nNumProjMatches,
                                                 std::vector<MapPoint*>& vpMPs,
                                                 std::vector<MapPoint*>& vpMatchedMPs)
{
  set<MapPoint*> spAlreadyMatchedMPs;
  nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw,
  ...
  if (nNumProjMatches >= nProjMatches)
  {
    ...
    if (mpTracker->mSensor == System::IMU_MONOCULAR && !pCurrentKF->GetMap()->GetIniertialBA2())
      bFixedScale = false;
    int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs,
                                             ...
                                             std::vector<MapPoint*>& vpMatchedMPs)
{
  ...
  for (KeyFrame* pKFi : vpBowCand)
  {
    ...
    for (int j = 0; j < vpCovKFi.size(); ++j)
    {
      ...
      // cout << "BoW: " << num << " putative matches with KF " << vpCovKFi[j]->mnId <<
      // endl;
    ...
    if (!bAbortByNearKF && numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      ...
      cout << "KFc: " << mpCurrentKF->mnId << "; KFm: " << pMostBoWMatchesKF->mnId <<
      endl;*/
      ...
      Sim3Solver solver = Sim3Solver(mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints,
      ...
      if (bConverge)
      {
        ...
        if (numProjMatches >= nProjMatches)
        {
          ...
          int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm,
                                                      10, mbFixScale, mHessian7x7, true);
          // cout <<"BoW: " << numOptMatches << " inliers in the Sim3 optimization" <<
          // endl;
          ...
          if (numOptMatches >= nSim3Inliers)
          {
            ...
            // cout <<"BoW: " << numProjOptMatches << " matches after of the Sim3
            // optimization" << endl;

            if (numProjOptMatches >= nProjOptMatches)
            {
              cout << "BoW: Current KF " << mpCurrentKF->mnId << "; candidate KF "
                   << pKFi->mnId << endl;
              ...
              // cout << "BoW: Coord in X -> " << min_x << ", " << max_x << "; and Y -> "
              // << min_y << ", " << max_y << endl;
              // cout << "BoW: features area in X -> " << (max_x - min_x) << " and Y -> "
              // << (max_y - min_y) << endl;
              ...
              // for(int j=0; j<vpCurrentCovKFs.size(); ++j)
              {
                ...
                if (bValid)
                {
                  // cout << "BoW: KF " << pKFj->mnId << " has " << numProjMatches_j << "
                  // matches" << endl;
  ...
  else
  {
    ...
    //        f_succes_pr << mpCurrentKF->mNameFile << " " << std::to_string(maxStage) <<
    //        endl;
    //        f_succes_pr << "% NumCand: " << std::to_string(numCandidates) << "; matches:
    //        " << std::to_string(maxMatched) << endl;
  }
  return false;
}

bool LoopClosing::DetectCommonRegionsFromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF,
                                                g2o::Sim3& gScw, int& nNumProjMatches,
                                                std::vector<MapPoint*>& vpMPs,
                                                std::vector<MapPoint*>& vpMatchedMPs)
{
  ...
  if (nNumProjMatches >= nProjMatches)
  {
    /*cout << "PR_From_LastKF: KF " << pCurrentKF->mnId << " has " << nNumProjMatches << "
    with KF " << pMatchedKF->mnId << endl;
    ...
    // cout << "PR_From_LastKF: Coord in X -> " << min_x << ", " << max_x << "; and Y -> "
    // << min_y << ", " << max_y << endl;
    // cout << "PR_From_LastKF: features area in X -> " << (max_x - min_x) << " and Y -> "
    // << (max_y - min_y) << endl;
...
int LoopClosing::FindMatchesByProjection(KeyFrame* pCurrentKF, KeyFrame* pMatchedKFw,
                                         g2o::Sim3& g2oScw, set<MapPoint*>& spMatchedMPinOrigin,
                                         vector<MapPoint*>& vpMapPoints,
...
void LoopClosing::CorrectLoop()
{
...

  {
    ...
    // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align
  ...
  {
    // cout << "With 7DoF" << endl;
    Optimizer::OptimizeEssentialGraph(pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3,
      ...
      (pLoopMap->KeyFramesInMap() < 200 && mpAtlas->CountMaps() == 1))
  {
    ...
    mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pLoopMap,
                             mpCurrentKF->mnId);
  }
...
void LoopClosing::MergeLocal()
{
  ...
  int numTemporalKFs = 15;  // TODO (set by parameter): Temporal KFs in the local window
                            // if the map is inertial.

  // Relationship to rebuild the essential graph, it is used two times, first in the local
  // window and later in the rest of the map
  KeyFrame* pNewChild;
  KeyFrame* pNewParent;
  ...
  // Flag that is true only when we stopped a running BA, in this case we need relaunch at
  ...
  // Merge map will become in the new active map with the local window of KFs and MPs from
  // the current map.
  // Later, the elements of the current map will be transform to the new active map
  ...
  Verbose::PrintMess("MERGE-VISUAL: Number of MPs in local window from active map: " +
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs)
  {
    ...
    // cout << "angle: " << 180*LogSO3(mg2oMergeScw.rotation().toRotationMatrix())/3.14 <<
    // endl;
    ...
    // cout << "After -> Map current: " << pCurrentMap << "; New map: " << pKFi->GetMap()
    // << endl;
    ...
    if (pCurrentMap->isImuInitialized())
    {
      ...
      // pKFi->SetVelocity(Converter::toCvMat(Rcor)*pKFi->GetVelocity()); // TODO: should
      // add here scale s
    }
  ...
  Verbose::PrintMess("MERGE-VISUAL: TRANSF ms: " + to_string(timeTransfMerge.count()),
                     Verbose::VERBOSITY_DEBUG);
  ...
  {
    unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate);  // We update the
                                                                   // current map with the
                                                                   // Merge information
    unique_lock<mutex> mergeLock(pMergeMap->mMutexMapUpdate);  // We remove the Kfs and
                                                               // MPs in the merged area
                                                               // from the old map
  ...
  Verbose::PrintMess("MERGE-VISUAL: New current map: " + to_string(pMergeMap->GetId()),
                     Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("MERGE-VISUAL: CRITICAL ms: " + to_string(timeCritMerge.count()),
                     Verbose::VERBOSITY_DEBUG);
  ...
  pNewParent = mpCurrentKF;  // Old child, now it will be the parent of its own parent(we
                             // need eliminate this KF from children list in its old
                             // parent)
  ...
  // Update connectivity
  Verbose::PrintMess("MERGE-VISUAL: Init to update connections in the welding area",
                     Verbose::VERBOSITY_DEBUG);
  for (KeyFrame* pKFi : spLocalWindowKFs)
  {
    if (!pKFi || pKFi->isBad())
  ...
  Verbose::PrintMess("MERGE-VISUAL: Finish to update connections in the welding area",
                     Verbose::VERBOSITY_DEBUG);
  ...
  if (mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO)
  {
    ...
    Optimizer::MergeInertialBA(mpLocalMapper->GetCurrKF(), mpMergeMatchedKF, &bStop,
                               mpCurrentKF->GetMap(), vCorrectedSim3);
  }
  else
  {
  ...
  {
    Verbose::PrintMess("MERGE-VISUAL: There are not KFs outside of the welding area",
                       Verbose::VERBOSITY_DEBUG);
  }
  else
  {
    ...
    // Optimize graph (and update the loop position for each element form the begining to
    // the end)
    if (mpTracker->mSensor != System::MONOCULAR)
    {
...

    {
      ...
      Verbose::PrintMess("MERGE-VISUAL: There are " +
                             to_string(pMergeMap->KeyFramesInMap()) + " KFs in the map",
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("MERGE-VISUAL: It will be inserted " +
                             to_string(vpCurrentMapKFs.size()) + " KFs in the map",
                         Verbose::VERBOSITY_DEBUG);
      ...
      Verbose::PrintMess("MERGE-VISUAL: There are " +
                             to_string(pMergeMap->MapPointsInMap()) + " MPs in the map",
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("MERGE-VISUAL: It will be inserted " +
                             to_string(vpCurrentMapMPs.size()) + " MPs in the map",
                         Verbose::VERBOSITY_DEBUG);
  ...
  if (bRelaunchBA && (!pCurrentMap->isImuInitialized() ||
                      (pCurrentMap->KeyFramesInMap() < 200 && mpAtlas->CountMaps() == 1)))
  {
    ...
    mbFinishedGBA = false;
    mbStopGBA = false;
    mpThreadGBA = new thread(&LoopClosing::RunGlobalBundleAdjustment, this, pMergeMap,
                             mpCurrentKF->mnId);
  }
                                         ...
                                         KeyFrame* mpCurrentKF, string& name)
{
  ...
  for (KeyFrame* pKFi : spLocalWindowKFs)
  {
    ...
    string filename_img = path_imgs + "KF" + to_string(mpCurrentKF->mnId) + "_" +
                          to_string(pKFi->mnId) + name + "points" +
...
void LoopClosing::MergeLocal2()
{
  ...
  pNewParent = mpMergeMatchedKF;      // Old child, now it will be the parent of its own
                                  // parent(we need eliminate this KF from children list
                                  // in its old parent)
  ...
  vector<MapPoint*> vpCheckFuseMapPoint;  // MapPoint vector from current map to allow to
                                          // fuse duplicated points with the old map
                                          // (merge)
    ...
    if (!mbStopGBA)
    {
      ...
      while (!lpKFtoCheck.empty())
      {
        ...
        for (set<KeyFrame*>::const_iterator sit = sChilds.begin(); sit != sChilds.end(); sit++)
        {
          ...
          {
            // cout << "++++New child with flag " << pChild->mnBAGlobalForKF << "; LoopKF:
            // " << nLoopKF << endl;
            ...
            cv::Mat Rcor =
                pChild->mTcwGBA.rowRange(0, 3).colRange(0, 3).t() * pChild->GetRotation();
        ...
        pKF->GetConnectedKeyFrames().size() << " connected KFs" << endl;
            cout << "--It has " << pKF->GetCovisiblesByWeight(80).size() << " connected KF
        with 80 common matches or more" << endl;
            cout << "--It has " << pKF->GetCovisiblesByWeight(50).size() << " connected KF
        with 50 common matches or more" << endl;
            cout << "--It has " << pKF->GetCovisiblesByWeight(20).size() << " connected KF
        with 20 common matches or more" << endl;
        ...
        desvZ << endl;
            ...
            for(int i=0; i<vpMapPointsKF.size(); ++i)
            {
                ...
                cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX,
        1, cv::Scalar(255, 0, 0));
            }
