...
f_track_stats << fixed ;*/
  ...
  f_track_times << "# ORB_Ext(ms), Stereo matching(ms), Preintegrate_IMU(ms), Pose pred(ms), LocalMap_track(ms), NewKF_dec(ms)"
                << endl;
  f_track_times << fixed;
#endif
}
                                  ...
                                  const double& timestamp, string filename)
{
  ...
  if (mSensor == System::STEREO && !mpCamera2)
    ...
    mCurrentFrame =
        Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight,
...
void Tracking::Track()
{
  ...
  if (mState != NO_IMAGES_YET)
  {
    ...
    else if (mCurrentFrame.mTimeStamp > mLastFrame.mTimeStamp + 1.0)
    {
      ...
      if (mpAtlas->isInertial())
      {
        ...
        {
          cout << "Timestamp jump detected. State set to LOST. Reseting IMU integration..."
               << endl;
  ...
  else
  {
    ...
    if (!mbOnlyTracking)
    {
      ...
      mTime_PosePred = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
    ...
    if (bOK || mState == RECENTLY_LOST)
    {
      ...
      mTime_NewKF_Dec = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(
      ...
      // with those points so we discard them in the frame. Only has effect if lastframe
      // is tracked
      for (int i = 0; i < mCurrentFrame.N; i++)
      {
        if (mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
...
void Tracking::MonocularInitialization()
{
  ...
  else
  {
    ...
    if (mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn, mCurrentFrame.mvKeysUn,
...
void Tracking::CreateMapInAtlas()
{
  ...
  mnLastRelocFrameId = mnLastInitFrameId;  // The last relocation KF_id is the current id,
                                           // because it is the new starting point for new
                                           // map
  ...
  for (int i = 0; i < nKFs; i++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        pSolver->SetRansacParameters(0.99, 10, 300, 6, 0.5,
...
void Tracking::CreateNewMapPoints()
{
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++)
    {
      ...
      if (bStereo1)
        cosParallaxStereo1 =
            cos(2 * atan2(mpLastKeyFrame->mb / 2, mpLastKeyFrame->mvDepth[idx1]));
      else if (bStereo2)
      ...
      const float ratioOctave =
          mpLastKeyFrame->mvScaleFactors[kp1.octave] / pKF2->mvScaleFactors[kp2.octave];
