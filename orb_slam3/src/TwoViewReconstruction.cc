                                        ...
                                        vector<bool> &vbTriangulated)
{
  ...
  thread threadH(&TwoViewReconstruction::FindHomography, this, ref(vbMatchesInliersH),
                 ref(SH), ref(H));
  thread threadF(&TwoViewReconstruction::FindFundamental, this, ref(vbMatchesInliersF),
                 ref(SF), ref(F));
  ...
  if (RH > 0.50)  // if(RH>0.40)
  {
    ...
    return ReconstructH(vbMatchesInliersH, H, mK, R21, t21, vP3D, vbTriangulated,
                        minParallax, 50);
  }
  else  // if(pF_HF>0.6)
  {
    // cout << "Initialization from Fundamental" << endl;
    return ReconstructF(vbMatchesInliersF, F, mK, R21, t21, vP3D, vbTriangulated,
                        minParallax, 50);
  }
}

void TwoViewReconstruction::FindHomography(vector<bool> &vbMatchesInliers, float &score,
                                           cv::Mat &H21)
{
...
void TwoViewReconstruction::FindFundamental(vector<bool> &vbMatchesInliers, float &score,
                                            cv::Mat &F21)
{
  // Number of putative matches
...
bool TwoViewReconstruction::ReconstructF(vector<bool> &vbMatchesInliers, cv::Mat &F21,
...
bool TwoViewReconstruction::ReconstructH(vector<bool> &vbMatchesInliers, cv::Mat &H21,
                                         cv::Mat &K, cv::Mat &R21, cv::Mat &t21,
                                         vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated,
                                         float minParallax, int minTriangulated)
{
  ...
  // Instead of applying the visibility constraints proposed in the Faugeras' paper (which
  // could fail for points seen with low parallax)
...
int TwoViewReconstruction::CheckRT(const cv::Mat &R, const cv::Mat &t,
                                   ...
                                   vector<bool> &vbGood, float &parallax)
{
  ...
  for (size_t i = 0, iend = vMatches12.size(); i < iend; i++)
  {
    ...
    // Check depth in front of first camera (only if enough parallax, as "infinite" points
