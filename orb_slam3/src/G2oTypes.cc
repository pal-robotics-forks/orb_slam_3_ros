...
Eigen::Vector3d EdgeMonoInvdepth::cam_unproject(const double u, const double v, const
double invDepth) const{
  Eigen::Vector3d res;
  res[2] = 1./invDepth;
  double z_x=res[2]/fx;
  double z_y=res[2]/fy;
...
Eigen::Vector2d EdgeMonoInvdepthBody::cam_project(const Eigen::Vector3d & trans_xyz)
const{
  double invZ = 1./trans_xyz[2];
  Eigen::Vector2d res;
  res[0] = invZ*trans_xyz[0]*fx + cx;
...
Eigen::Vector3d EdgeMonoInvdepthBody::cam_unproject(const double u, const double v, const
double invDepth) const{
  Eigen::Vector3d res;
  res[2] = 1./invDepth;
  double z_x=res[2]/fx;
...
void EdgeInertial::computeError()
{
  ...
  const Eigen::Vector3d ev =
      VP1->estimate().Rwb.transpose() * (VV2->estimate() - VV1->estimate() - g * dt) - dV;
...
void EdgeInertial::linearizeOplus()
{
  ...
  _jacobianOplus[0].block<3, 3>(6, 0) =
      Skew(Rbw1 * (VP2->estimate().twb - VP1->estimate().twb - VV1->estimate() * dt -
                   0.5 * g * dt * dt));  // OK
  // translation
...
void EdgeInertialGS::linearizeOplus()
{
  ...
  _jacobianOplus[0].block<3, 3>(6, 0) =
      Skew(Rbw1 * (s * (VP2->estimate().twb - VP1->estimate().twb - VV1->estimate() * dt) -
                   0.5 * g * dt * dt));
  // translation
