      ...
      for (size_t iKF = 0; iKF < vIndicesKF.size(); iKF++)
      {
        ...
        if (bestDist1 <= TH_LOW)
        {
          if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2))
          {
            ...
            const cv::KeyPoint &kp = (!pKF->mpCamera2) ?
                                       ...
                                       const bool bOnlyStereo, const bool bCoarse)
{
  ...
  else
  {
    ...
    tll = pKF1->GetRotation() * (-pKF2->GetRotation().t() * pKF2->GetTranslation()) +
          pKF1->GetTranslation();
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints,
  ...
  for (int i = 0; i < LastFrame.N; i++)
  {
    ...
    if (pMP)
    {
      if (!LastFrame.mvbOutlier[i])
      {
        ...
        {
          cv::Mat x3Dr = CurrentFrame.mTrl.colRange(0, 3).rowRange(0, 3) * x3Dc +
          ...
          if (bForward)
            vIndices2 =
                CurrentFrame.GetFeaturesInArea(uv.x, uv.y, radius, nLastOctave, -1, true);
          else if (bBackward)
