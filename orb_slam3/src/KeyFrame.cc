...
void KeyFrame::SetBadFlag()
{
  // std::cout << "Erasing KF..." << std::endl;
  {
    ...
    if (!mpParent)
    {
      // cout << "KF.BADFLAG-> There is not parent, but it is not the first KF in the map"
      // << endl;
      // cout << "KF.BADFLAG-> KF: " << mnId << "; first KF: " << mpMap->GetInitKFid() <<
      // endl;
    }
  }
...

  {
    ...
    // std::cout << "KF.BADFLAG-> Initially there are " << sParentCandidates.size() << "
    // candidates" << std::endl;
    ...
    while (!mspChildrens.empty())
    {
      ...
      if (bContinue)
      {
        if (pC->mnId == pP->mnId)
        {
          /*cout << "ERROR: The parent and son can't be the same KF. ID: " << pC->mnId <<
          endl;
    ...
    {
      // cout << "Error: KF haven't got a parent, it is imposible reach this code point
      // without him" << endl;
    }
    mbBad = true;
  }
                                           ...
                                           const bool bRight) const
{
  ...
  const int nMaxCellX =
      min((int)mnGridCols - 1, (int)ceil((x - mnMinX + factorX) * mfGridElementWidthInv));
  if (nMaxCellX < 0)
    return vIndices;
  ...
  const int nMaxCellY =
      min((int)mnGridRows - 1, (int)ceil((y - mnMinY + factorY) * mfGridElementHeightInv));
  if (nMaxCellY < 0)
    return vIndices;
