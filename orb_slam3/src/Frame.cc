  ...
  , mTimeORB_Ext(0)
{
  ...
  // imGray(cv::Range(0,175),cv::Range(0,512)) = cv::Mat::zeros(cv::Size(512,
  // 175),CV_8UC1);
  // imGray(cv::Range(350,512),cv::Range(260,360)) = cv::Mat::zeros(cv::Size(100,
  // 162),CV_8UC1);
...
void Frame::UndistortKeyPoints()
{
  ...
  // Undistort points
  mat = mat.reshape(2);
  cv::undistortPoints(mat, mat, static_cast<Pinhole *>(mpCamera)->toK(), mDistCoef,
                      cv::Mat(), mK);
  mat = mat.reshape(1);
  ...
  for (vector<vector<cv::DMatch>>::iterator it = matches.begin(); it != matches.end(); ++it)
  {
    if ((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7)
    {
      ...
      float depth = static_cast<KannalaBrandt8 *>(mpCamera)->TriangulateMatches(
          ...
          mvKeysRight[(*it)[0].trainIdx + monoRight], mRlr, mtlr, sigma1, sigma2, p3D);
      if (depth > 0.0001f)
      {
