  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    else
    {
      ...
      if (dist > 1)
      {
        ...
        Verbose::PrintMess("GBA: KF " + to_string(pKF->mnId) + " had been moved " +
                               to_string(dist) + " meters",
        ...
        Verbose::PrintMess("--Number of discarded observations: " + to_string(numMonoBadPoints) +
                               " in mono and " + to_string(numStereoBadPoints) + " in stereo",
                           Verbose::VERBOSITY_DEBUG);
      }
    }
  }
...
void Optimizer::FullInertialBA(Map* pMap, int its, const bool bFixLocal,
                               ...
                               bool bInit, float priorG, float priorA,
    ...
    if (pKFi->mPrevKF && pKFi->mnId <= maxKFid)
    {
      ...
      if (pKFi->bImu && pKFi->mPrevKF->bImu)
      {
        ...
        if (!bInit)
        {
          ...
          egr->setVertex(1, VG2);
          cv::Mat cvInfoG =
              pKFi->mpImuPreintegrated->C.rowRange(9, 12).colRange(9, 12).inv(cv::DECOMP_SVD);
          ...
          ear->setVertex(1, VA2);
          cv::Mat cvInfoA =
              pKFi->mpImuPreintegrated->C.rowRange(12, 15).colRange(12, 15).inv(cv::DECOMP_SVD);
...
int Optimizer::PoseOptimization(Frame* pFrame)
{
  ...
  // inlier/outlier
  // At the next optimization, outliers are not included, but at the end they can be
  // classified as inliers again.
  ...
  // cout << "[PoseOptimization]: initial correspondences-> " << nInitialCorrespondences
  // << " --- outliers-> " << nBad << endl;
                                      ...
                                      vector<KeyFrame*>& vpNonEnoughOptKFs)
{
  ...
  // Verbose::PrintMess("LM-LBA: There are " + to_string(lLocalKeyFrames.size()) + " KFs
  // and " + to_string(lLocalMapPoints.size()) + " MPs to optimize. " +
  ...
  Verbose::PrintMess("LM-LBA: opt/fixed KFs: " + to_string(lLocalKeyFrames.size()) + "/" +
       ...
       lit != lend; lit++)
  {
         ...
         mit != mend; mit++)
    {
      ...
      if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
      {
        ...
        else if (cam0Index != -1 && pKFi->mvuRight[cam0Index] >= 0)  // Stereo observation
                                                                     // (with rectified
                                                                     // images)
        {
          const cv::KeyPoint& kpUn = pKFi->mvKeysUn[cam0Index];
  ...
  // << std::endl;
  // std::cout << "Keyframes: " << nKFs << " --- MapPoints: " << nPoints << " --- Edges: "
  // << nEdges << endl;
  ...
  if (bDoMore)
  {
    ...
    Verbose::PrintMess("LM-LBA: First optimization has " + to_string(nMonoBadObs) +
                           " monocular and " + to_string(nStereoBadObs) + " stereo bad observations",
                       Verbose::VERBOSITY_DEBUG);
  ...
  Verbose::PrintMess("LM-LBA: outlier observations: " + to_string(vToErase.size()),
                     Verbose::VERBOSITY_DEBUG);
  Verbose::PrintMess("LM-LBA: total of observations: " +
  ...
  if (!vToErase.empty())
  {
    // cout << "LM-LBA: There are " << vToErase.size() << " observations whose will be
    // deleted from the map" << endl;
       ...
       lit != lend; lit++)
  {
    ...
    // cout << "LM-LBA: KF " << pKFi->mnId << " had performed " <<  pKFi->mnNumberOfOpt <<
    // " iterations" << endl;
  ...
  // Verbose::PrintMess("LM-LBA: Num fixed cameras " + to_string(num_fixedKF),
  // Verbose::VERBOSITY_DEBUG);
  // Verbose::PrintMess("LM-LBA: Num Points " + to_string(lLocalMapPoints.size()),
  // Verbose::VERBOSITY_DEBUG);
  // Verbose::PrintMess("LM-LBA: Num optimized cameras " +
  // to_string(lLocalKeyFrames.size()), Verbose::VERBOSITY_DEBUG);
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF)
{
  ...
  // Verbose::PrintMess("LM-LBA: opt/fixed KFs: " + to_string(lLocalKeyFrames.size()) +
  // "/" + to_string(lFixedCameras.size()), Verbose::VERBOSITY_DEBUG);
  // Verbose::PrintMess("LM-LBA: local MPs: " + to_string(lLocalMapPoints.size()),
  // Verbose::VERBOSITY_DEBUG);
  ...
  if (vToErase.size() >= (vpMapPointEdgeMono.size() + vpMapPointEdgeStereo.size()) * 0.5)
  {
    ...
    /*for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        ...
        Verbose::PrintMess("ERROR CHI2: " + to_string(e->chi2()) + "; DEPTH POSITIVE: " +
    to_string(e->isDepthPositive()), Verbose::VERBOSITY_NORMAL);
    }*/
  ...
  if (!vToErase.empty())
  {
    ...
    if (bRedrawError)
    {
      ...
      f_lba << "# KF id, Initial Num CovKFs, Final Num CovKFs, Initial Num MPs, Fimal Num MPs"
            << endl;
      f_lba << fixed;
       ...
       lit != lend; lit++)
  {
    ...
    {
      bShowStats = true;
      Verbose::PrintMess("LM-LBA: Too much distance in KF " + to_string(pKFi->mnId) + ", " +
                             to_string(dist) + " meters. Current KF " + to_string(pKF->mnId),
                         Verbose::VERBOSITY_DEBUG);
      ...
      Verbose::PrintMess("LM-LBA: Good observations in mono " + to_string(numMonoMP) +
                             " and stereo " + to_string(numStereoMP),
                         Verbose::VERBOSITY_DEBUG);
      Verbose::PrintMess("LM-LBA: Bad observations in mono " + to_string(numBadMonoMP) +
                             " and stereo " + to_string(numBadStereoMP),
                         Verbose::VERBOSITY_DEBUG);
    }
  }
  // Verbose::PrintMess("LM-LBA: Num fixed cameras " + to_string(num_fixedKF),
  ...
  for (KeyFrame* pKFi : vpNonFixedKFs)
  {
    ...
    /*{
        ...
        if(dist > 1.0)
        {
            ...
            cout << "--To much distance correction in EssentGraph: It has connected " <<
    pKFi->GetVectorCovisibleKeyFrames().size() << " KFs" << endl;
        }
        ...
        for(int j=0; j<vpMapPointsKFi.size(); ++j)
        {
            ...
            cv::putText(imLeft, strNumOBs, pKFi->mvKeys[j].pt, CV_FONT_HERSHEY_DUPLEX, 1,
    cv::Scalar(255, 0, 0));
        }

        string namefile = "./test_OptEssent/Essent_" + to_string(pCurKF->mnId) + "_KF" +
    to_string(pKFi->mnId) +"_D" + to_string(dist) +".png";
                                       ...
                                       const LoopClosing::KeyFrameAndPose& CorrectedSim3)
{
  ...
  Verbose::PrintMess("edges pose graph: " + to_string(count_edges[0]) + ", " +
                         to_string(count_edges[1]) + ", " + to_string(count_edges[2]),
                     Verbose::VERBOSITY_DEBUG);
  // Optimize!
  optimizer.initializeOptimization();
  optimizer.setVerbose(false);
                            ...
                            Eigen::Matrix<double, 7, 7>& mAcumHessian, const bool bAllPoints)
{
  ...
  Verbose::PrintMess("Sim3: There are " + to_string(nCorrespondences) + " matches, " +
  ...
  Verbose::PrintMess("Sim3: First Opt -> Correspondences: " + to_string(nCorrespondences) +
                         "; nBad: " + to_string(nBad) + "; nBadOutKF2: " + to_string(nBadOutKF2),
                     Verbose::VERBOSITY_DEBUG);
                            ...
                            Eigen::Matrix<double, 7, 7>& mAcumHessian, const bool bAllPoints)
{
  ...
  // cout << "Sim3 -> Correspondences: " << nCorrespondences << "; nBad: " << nBad <<
  // endl;
...
void Optimizer::LocalInertialBA(KeyFrame* pKF, bool* pbStopFlag, Map* pMap, bool bLarge, bool bRecInit)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
    {
      ...
      if (i == N - 1 || bRecInit)
      {
        // All inertial residuals are included without robust cost function, but not that
        // one linking the
        // last optimizable keyframe inside of the local window and the first fixed
        // keyframe out. The
        // information matrix for this measurement is also downweighted. This is done to
        // avoid accumulating
  ...
  /*double t_const = std::chrono::duration_cast<std::chrono::duration<double> >(t1 -
  t0).count();
  double t_opt = std::chrono::duration_cast<std::chrono::duration<double> >(t2 -
  t1).count();
...
void Optimizer::InertialOptimization(Map* pMap, Eigen::Vector3d& bg, Eigen::Vector3d& ba,
...
void Optimizer::InertialOptimization(vector<KeyFrame*> vpKFs, Eigen::Vector3d& bg,
...
void Optimizer::LocalBundleAdjustment(KeyFrame* pMainKF, vector<KeyFrame*> vpAdjustKF,
                                      vector<KeyFrame*> vpFixedKF, bool* pbStopFlag)
{
  ...
  // Verbose::PrintMess("LBA: There are " + to_string(vpMPs.size()) + " MPs to optimize",
  // Verbose::VERBOSITY_NORMAL);
  ...
  // Verbose::PrintMess("LBA: number total of edged -> " + to_string(vpEdgeKFMono.size() +
  // vpEdgeKFStereo.size()), Verbose::VERBOSITY_NORMAL);
  ...
  if (!vToErase.empty())
  {
    ...
    for (KeyFrame* pKFi : spErasedKFs)
    {
      ...
      Verbose::PrintMess("LBA: Initially KF " + to_string(pKFi->mnId) + " had " +
                             to_string(num_init_MPs) + ", at the end has " + to_string(num_MPs),
                         Verbose::VERBOSITY_DEBUG);
    }
  }
  for (unsigned int i = 0; i < vpMPs.size(); ++i)
  ...
  for (KeyFrame* pKFi : vpAdjustKF)
  {
    ...
    {
      Verbose::PrintMess("LBA ERROR: KF " + to_string(pKFi->mnId) + " has only " +
    ...
    if (dist > 1.0)
    {
      if (bShowImages)
      {
        ...
        for (int i = 0; i < vpMonoMPsOpt.size(); ++i)
        {
          ...
          if (index < 0)
          {
            // cout << "LBA ERROR: KF has a monocular observation which is not recognized
            // by the MP" << endl;
            // cout << "LBA: KF " << pKFi->mnId << " and MP " << vpMonoMPsOpt[i]->mnId <<
            // " with index " << endl;
            continue;
          }
        ...
        for (int i = 0; i < vpStereoMPsOpt.size(); ++i)
        {
          ...
          if (index < 0)
          {
            // cout << "LBA: KF has a stereo observation which is not recognized by the
            // MP" << endl;
            // cout << "LBA: KF " << pKFi->mnId << " and MP " << vpStereoMPsOpt[i]->mnId
            // << endl;
            continue;
          }
        ...
        Verbose::PrintMess("--To much distance correction in LBA: It has " +
                           ...
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--To much distance correction in LBA: It has " +
                               to_string(mpObsFinalKFs[pKFi]) + " deleted observations",
                           Verbose::VERBOSITY_DEBUG);
        Verbose::PrintMess("--------", Verbose::VERBOSITY_DEBUG);
      }
    }
...
void Optimizer::MergeInertialBA(KeyFrame* pCurrKF, KeyFrame* pMergeKF, bool* pbStopFlag,
                                Map* pMap, LoopClosing::KeyFrameAndPose& corrPoses)
{
  const int Nd = 6;
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame* pFrame, bool bRecInit)
{
  ...
  egr->setVertex(1, VG);
  cv::Mat cvInfoG =
      pFrame->mpImuPreintegrated->C.rowRange(9, 12).colRange(9, 12).inv(cv::DECOMP_SVD);
  Eigen::Matrix3d InfoG;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++)
      InfoG(r, c) = cvInfoG.at<float>(r, c);
  ...
  ear->setVertex(1, VA);
  cv::Mat cvInfoA =
      pFrame->mpImuPreintegrated->C.rowRange(12, 15).colRange(12, 15).inv(cv::DECOMP_SVD);
  Eigen::Matrix3d InfoA;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++)
      InfoA(r, c) = cvInfoA.at<float>(r, c);
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame* pFrame, bool bRecInit)
{
  ...
  ear->setVertex(1, VA);
  cv::Mat cvInfoA =
      pFrame->mpImuPreintegratedFrame->C.rowRange(12, 15).colRange(12, 15).inv(cv::DECOMP_SVD);
  Eigen::Matrix3d InfoA;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++)
      InfoA(r, c) = cvInfoA.at<float>(r, c);
  ...
  pFrame->mpcpi =
      new ConstraintPoseImu(VP->estimate().Rwb, VP->estimate().twb, VV->estimate(),
                            VG->estimate(), VA->estimate(), H.block<15, 15>(15, 15));
  delete pFp->mpcpi;
  pFp->mpcpi = NULL;
