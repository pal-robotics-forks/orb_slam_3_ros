  ...
  , infoInertial(Eigen::MatrixXd::Zero(9, 9))
{
  ...
  f_lm << "# Timestamp KF, Num CovKFs, Num KFs, Num RecentMPs, Num MPs, processKF,
  MPCulling, CreateMP, SearchNeigh, BA, KFCulling, [numFixKF_LBA]" << endl;
  f_lm << fixed;*/
}
...
void LocalMapping::Run()
{
  ...
  while (1)
  {
    ...
    if (CheckNewKeyFrames() && !mbBadImu)
    {
        ...
        if (mpAtlas->KeyFramesInMap() > 2)
        {
          if (mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
          {
            ...
            Optimizer::LocalInertialBA(mpCurrentKeyFrame, &mbAbortBA,
                                       mpCurrentKeyFrame->GetMap(), bLarge,
      ...
      double t_KF_cull =
          std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t6 - t5).count();
      double t_Insert =
          std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t8 - t7).count();
...
void LocalMapping::CreateNewMapPoints()
{
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++)
  {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++)
    {
      ...
      const bool bRight1 =
          (mpCurrentKeyFrame->NLeft == -1 || idx1 < mpCurrentKeyFrame->NLeft) ? false : true;
...
void LocalMapping::KeyFrameCulling()
{
  ...
  const int Nd =
      21;  // MODIFICATION_STEREO_IMU 20 This should be the same than that one from LIBA
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
  ...
  {
    if (priorA != 0.f)
      Optimizer::FullInertialBA(mpAtlas->GetCurrentMap(), 100, false, 0, NULL, true,
                                priorG, priorA);
    else
  ...
  cout << "ba: " << mpCurrentKeyFrame->GetAccBias() << endl;
  cout << "bg: " << mpCurrentKeyFrame->GetGyroBias() << endl;
  double t_inertial_only = std::chrono::duration_cast<std::chrono::duration<double> >(t1 -
  t0).count();
  double t_update = std::chrono::duration_cast<std::chrono::duration<double> >(t3 -
  t2).count();
  double t_viba = std::chrono::duration_cast<std::chrono::duration<double> >(t5 -
  t4).count();
...
void LocalMapping::ScaleRefinement()
{
  ...
  // the difference between monocular and stereo
